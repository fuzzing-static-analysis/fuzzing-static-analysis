...
impl LLVMPasses {
    ...
    pub fn path(&self) -> PathBuf {
        match self {
            ...
            LLVMPasses::Analysis => {
                PathBuf::from(env!("OUT_DIR")).join(format!("analysis.{}", dll_extension()))
            }
            LLVMPasses::DDG => {
                PathBuf::from(env!("OUT_DIR")).join(format!("ddg_instr.{}", dll_extension()))
            }
            LLVMPasses::Table => {
                PathBuf::from(env!("OUT_DIR")).join(format!("table.{}", dll_extension()))
            }
        }
    }
}
