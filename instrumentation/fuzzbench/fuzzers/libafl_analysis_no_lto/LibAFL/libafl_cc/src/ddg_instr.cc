...
class DDGInstrModulePass : public PassInfoMixin<DDGInstrModulePass> {
 private:
  void DumpDataFlow(std::string genericFilePath) {
    std::ofstream out;
    if (getenv("DDG_OUTPUT_PATH")) {
      std::string fname = getenv("DDG_OUTPUT_PATH") + std::string("/") +
                          genericFilePath + ".ddg";
      out = std::ofstream(fname);
    } else {
      errs() << "ddg output path not set!"
             << "\n";
      return;
    }
    ...
    while (it != this->DataFlowTracker.end()) {
      ...
      StringRef fnname = this->ValueToFnname[it->first];
      if (out << std::string(fnname) << " " << it->first << " "
              << "\n"
              << "deps: ") {
      } else {
        abort();
  ...
  void CreateDataFlow(StringRef fnname, Value *Variable) {
    std::map<Value *, std::vector<Value *>>::iterator it =
        this->DataFlowTracker.find(Variable);
    if (it == this->DataFlowTracker.end()) {
      this->DataFlowTracker[Variable].push_back(Variable);
      this->ValueToFnname[Variable] = fnname;
    }
  }
 ...
 public:
  ...
  std::map<Value *, StringRef>            ValueToFnname;
  ...
  */
  ...
  PreservedAnalyses run(Module &M, ModuleAnalysisManager &MAM) {
    ...
    */
    ...
    bool done_already = file_exist("/out/." + genericFilePath + ".ddg");
    if (done_already) {
      run = false;
    } else {
      std::ofstream out_lock("/out/." + genericFilePath + ".ddg");
    }

    if (run) {
      outs() << "DDG on " + genericFilePath << "\n";
      auto &FAM =
      ...
      for (auto &F : M) {
        ...
        std::map<Value *, Instruction *>
            LLVMVariables;  // LLVM IR Variables which are used as Operands for
                            // the store (for instance, the ones resulting from
                            // a GEP)
        ...
        for (auto &BB : F) {
          ...
          for (auto &I : BB) {
            ...
            if (auto AI = dyn_cast<AllocaInst>(&I)) {
              ...
              auto   fnname = F.getName();
              CreateDataFlow(fnname, Variable);
            }
            ...
            if (auto LOI = dyn_cast<LoadInst>(&I)) {
              ...
              // If `Variable` does not directly represent a Src code variable,
              // we fetch what it represents (e.g., the field of a struct)
              ...
              for (std::vector<Value *>::iterator it = Flows.begin();
                ...
                // ReachingStores[0] refers to the last Store instruction that
                // we met (i.e., the last in `AllStoresPerVariable` This is why
                // we iterate the vector in a reverse way BUT the array in the
                // forward
                unsigned i = 0;
                for (std::vector<FlowWriteInstruction *>::reverse_iterator it =
                  ...
                  if (ReachingStores[i] && (i < ConsideredStores)) {
                    ...
                    if (Src == LOI)  // Already managed in the
            ...
            if (auto ST = dyn_cast<StoreInst>(&I)) {
              ...
              Value *Access =
                  ST->getValueOperand();  // What we're writing, this
              ...
              for (std::vector<Value *>::iterator it = Flows.begin();
                ...
                if (Dependency == Variable)  // If we fall into `a += c + b`, we
                                             // manage differently
                  Type = modification;       // Probably we dont need this
                                        // distinction anymore, but keep it for
                                        // future experiments
                ...
                for (std::vector<FlowWriteInstruction *>::reverse_iterator it =
                  ...
                  if (ReachingStores[i] && (i < ConsideredStores)) {
                    Instruction *Src = (*it)->I;
                    if (Src == ST)  // Already managed in the
                                    // `reachableByStores` method
                      continue;
                    if (isPredecessorBB(
                            Src,
            ...
            // Three major cases:
            // 1) a = foo(x)           => a depends on the result of foo()
            // applied on x and x depends on its previous values and return
            // value 2) memcpy(src, dst, N)  => dst depends on src and N && the
            // triple src, dst, N depends on their previous value (memcpy or any
            // other API) 3) foo(x, out_y, out_z) => out_x, out_y are writen
            // within foo depending on x. Thus here the dependency is managed
            // internally to the function when passing on it
              ...
              int argStart =
                  0;  // In some cases, we dont want to track dependencies for
                      // each argument. For instance, for memcpy(src, dst, n),
                      // we can ignore previous `src` dependencies, since it is
                      // being written. Rather, for this specific case, we
                      // generate a FlowWriteInstruction object to save the fact
                      // that `src` internal value has been modified according
                      // to `dst` and `n`
...
llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "DDGInstrModulePass", "v0.1",
          [](PassBuilder &PB) {
